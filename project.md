...
- **2026-08-29 ~ 2026-08-30 (全栈重构与企业级增强演进)**：
  ...
  18. **机器上下线告警通知 (Telegram & SMTP)**：系统设置中心新增上下线告警通知选项卡，完美复刻现代化 shadcn 选项卡布局；支持防抖判定（连续失败阈值、累计断线时长阈值、静默防刷屏冷却）；支持 Telegram Bot 即时消息与 SMTP 邮件多收件人批量推送；提供一键发送测试消息/邮件连通性诊断；服务端守护进程每 10 秒高频巡检并智能分发下线告警与上线恢复通知。
  19. **服务器到期时间与价格管理 + 3/7 天到期告警预警**：保持原版经典 shadcn/ui 扁平极简风格；在数据库 `nodes` 表扩展 `price` (价格/周期) 与 `expire_time` (到期毫秒时间戳) 持久化字段；节点创建、编辑与自动发现认领表单全面支持录入；前端网格卡片、表格视图与单机详情页直观展示价格徽章与到期倒计时预警；告警引擎 (`notify.js`) 新增提前 7 天（温馨提示）与提前 3 天（紧急预警）防重通知机制，通过 Telegram 机器人与 SMTP 邮件提醒管理员及时续费。
